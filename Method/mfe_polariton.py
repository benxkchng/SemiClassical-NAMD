...
def runTraj(parameters):
    ...
    for itraj in range(NTraj): 
        ...
        for i in range(NSteps): # One trajectory
            #------- ESTIMATORS-------------------------------------
            if (i % nskip == 0):
                # Segment below produces population in diabatic basis
                #rho_ensemble[:,:,iskip] += pop(dat)
                #iskip += 1

                # Segment below produces population in polariton eigenspectrum
                # ,i.e adiabatic basis 
                eValue,eVector = np.linalg.eig(parameters.Hel(dat.R))
                rho_ensemble[:,:,iskip] += pop(dat)
                
                # Diagonalization of density matrix
                rho_ensemble[:,:,iskip] = eVector @ rho_ensemble[:,:,iskip] @ np.conj(eVector).T
                
                # Rearranging basis of polaritonic spectrum from UP to LP state
                populations = np.diag(rho_ensemble[:,:,iskip])
                eValue_reshape = np.reshape(eValue,(len(eValue),1))
                pops_reshape = np.reshape(populations,(len(populations),1)) 
                vector = np.concatenate((eValue_reshape,pops_reshape),axis=1)
                vector_index = np.argsort(vector[:,0])
                sorted_array = vector[vector_index]
                populations = sorted_array[:,1]
                rho_ensemble[:,:,iskip] = np.diag(populations)
                iskip += 1
            #-------------------------------------------------------
            dat = vv(dat)

    return rho_ensemble
    #return rho_polariton_ensemble
