...
class parameters():
   ...
   M = 1
   initState = 17 # first excited mode  # photon mode 2.448 eV
   ...
   Lz    = (3.3*10**-6) * Nmols/(0.528*10**-10) # 5*10**-6/(0.528*10**-10)  #100 * Nmols # Nmolecule in Lz Figure out
   ...
   gc0  = 0.0028/(Nmols)**0.5 # 0.1 is 100 meV
...
def Hel(R):
    ...
    for jState in range(Nmol):
        ...
        #print(np.sum( ωm[:]**2  * R[jState * nPhonon : (jState + 1) * nPhonon] * R0[:])*27.2114)
        #print("Check R state is ", R[jState * nPhonon : (jState + 1) * nPhonon])
        #print(R[jState * nPhonon : (jState + 1) * nPhonon])
        #print(R0[:])
        # b^2 term
